...
public class MicroProfileOpenApi20Test {
    ...
    public void testOpenApiDocumentForV20Annotations(@ArquillianResource URL baseURL) throws URISyntaxException {
        ...
        Assert.assertFalse("\"/districts/{code20}\" property is empty", getDistrictByCode20Path.isEmpty());

        Map<String, Object> patchMethod = (Map<String, Object>) getDistrictByCodePath.get("patch");
        Assert.assertFalse("\"/districts/{code20}\" \"patch\" property is empty", patchMethod.isEmpty());
        Assert.assertEquals("\"/districts/{code20}\" \"patch\" property should have exactly 13 keys",
                13, patchMethod.keySet().size());

        Map<String, Object> requestBody = (Map<String, Object>) patchMethod.get("requestBody");
        Assert.assertFalse("\"/districts/{code20}\" \"requestBody\" for PATCH verb is empty", requestBody.isEmpty());

        Map<String, Object> content = (Map<String, Object>) requestBody.get("content");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"requestBody\" for PATCH verb has empty \"content\" property",
                content.isEmpty());

        Map<String, Object> contentJson = (Map<String, Object>) content.get("application/json");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"response\" for PATCH verb has \"content\" but empty \"application/json\" property",
                contentJson.isEmpty());

        Map<String, Object> requestBodySchema = (Map<String, Object>) contentJson.get("schema");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"response\" for GET verb has \"application/json\" but empty \"schema\" property",
        ...
        Assert.assertFalse("\"/districts/{code20}\" \"responses\" for PATCH verb is empty", patchResponses.isEmpty());

        Map<String, Object> patchHttp200Response = (Map<String, Object>) patchResponses.get("200");
        Assert.assertFalse("\"/districts/{code20}\" \"response\" for PATCH verb and HTTP status 200 is empty",
                patchHttp200Response.isEmpty());

        Map<String, Object> patchContentAnnotation = (Map<String, Object>) patchHttp200Response.get("content");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"response\" for PATCH verb and HTTP status 200 has empty \"content\" property",
                patchContentAnnotation.isEmpty());

        Map<String, Object> patchContentTypeJson = (Map<String, Object>) patchContentAnnotation.get("application/json");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"response\" for PATCH verb and HTTP status 200 has \"content\" but empty \"application/json\" property",
                patchContentTypeJson.isEmpty());

        Map<String, Object> responseSchema = (Map<String, Object>) patchContentTypeJson.get("schema");
        Assert.assertFalse(
                "\"/districts/{code20}\" \"response\" for PATCH verb has \"application/json\" but empty \"schema\" property",
        ...
        // let's check whether /districts/{code20} response for GET verb and HTTP status 200 has MockString reference in it's DistrictEntity scheme "code" property value
